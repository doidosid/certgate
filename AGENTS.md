...
- 사용자가 리뷰 내용을 파일로 요청하면 저장 위치는 저장소 루트의 `codexReview/` 폴더로 한다.
- PR 리뷰 문서 파일명은 `PR-{번호}.md`를 기본으로 한다. 사용자가 특정 파일명을 지정하면 그 이름을 사용하되 `codexReview/` 아래에 둔다.
- `codexReview/`와 루트의 기존 `codexReview.md`는 로컬 리뷰 산출물이며 Git에 추가·커밋·푸시하지 않는다.
- 리뷰 문서에는 검토 대상, 심각도별 문제, 근거 위치, 영향, 권장 수정, 테스트 결과와 추가 테스트를 포함한다.
