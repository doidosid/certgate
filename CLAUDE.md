...
**리뷰 요청·반영·Merge는 Claude가 자동으로 진행한다.** 사용자에게 매 단계 확인을 받지 않는다.
...
**Codex의 지적을 무비판적으로 그대로 반영하지 않는다.** 각 지적을 이 저장소의 문서·ADR·실제 코드와 대조해 사실인지 먼저 확인한다. 가능하면 재현하거나 테스트로 검증한다.

- 사실이고 이 프로젝트 기준에 맞으면 반영한다.
- 사실이 아니거나 이 저장소의 결정과 어긋나면 근거를 들어 반박하고 반영하지 않는다. 반박한 내용은 PR 코멘트나 Commit 메시지에 남겨 판단 근거가 사라지지 않게 한다.
- 사실이지만 지금 다룰 범위가 아니면(다른 Issue의 영역, 마감 우선순위) Issue로 분리하고 그 판단을 기록한다.

Codex가 놓친 문제를 Claude가 발견하면 그것도 함께 고친다. 리뷰는 판단의 입력이지 판단 자체가 아니다.
