...
export function usePageParams() {
	...
	// 사용자가 URL을 직접 고칠 수 있으므로 숫자가 아니거나 음수인 값은 기본값으로
	// 되돌린다. 그대로 두면 page=abc가 NaN이 되어 요청과 표가 함께 깨진다.
	const page = positiveIntOr(searchParams.get("page"), 0);
	const size = positiveIntOr(searchParams.get("size"), DEFAULT_SIZE);
...
function positiveIntOr(raw: string | null, fallback: number): number {
	if (raw === null) {
		return fallback;
	}
