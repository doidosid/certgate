 ...
 * 상세로 이동".
 *
 * Security Events 화면에는 이 Event 하나만 여는 상세 화면이 없다 — 목록 + Drawer뿐이다
 * (Task 8). 그래서 "상세로 이동"을 목록으로 이동해 같은 reasonCode로 필터링하는 것으로
 * 구현한다. `usePageParams`가 읽는 것과 같은 Query Key라 필터가 그대로 적용된다.
 *
	...
	return (
		<Stack spacing={1.5}>
			{events.map((event) => (
				<RouterLink
					key={event.id}
					to={`/security-events?reasonCode=${encodeURIComponent(event.reasonCode)}`}
